...
import { setlockScreen ,appStack} from '../../../navigation/layouts';
...
class VerificationScreen extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (prevProps.fetching && !this.props.fetching) {
      console.tron.log('props',prevProps)
      if (this.props.error) {
        Alert.alert('Sorry Invalid Verification Number');
      } else {
  ...
  if (prevProps.fetchingResend && !this.props.fetchingResend) {
    if (this.props.errorResend) {
      Alert.alert('Sorry Didn;t resent');
    } else {
     if(!prevProps.resend){
  ...
  render() {
     let textStyle = this.state.enterCode ? {
        ...
        const  {  registerAccount, verified } = this.props
            ...
            //console.tron.log("onsubmit",state.params.username)
      ...
      return(
        ...
        <View style={styles.container}>
         <Card style={[styles.card,{justifyContent:'center'}]}>
            <View animation="bounceInLeft" delay={30}>

            <LottieView source={Animation.done}  autoPlay loop style={styles.animation} />
           <Text style={styles.verified}>Your Telephone Is Verified</Text>
          <CardItem>
               <Button block light style={styles.button} onPress={this.registerationDone}>
